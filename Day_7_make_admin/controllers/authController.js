...
const login=async(req,res)=>{
    ...
    try{
        ...
        const token=jwt.sign({id:user._id,email,role:user.role},process.env.TOKEN_SECRET);
        ...
        return res.status(201).send({message:"User is Authenticated",token});
...
const signup=async (req,res)=>{
    ...
    if(user) return res.status(401).send(`User with ${email} already exists`);
